...
class DreamDate extends DreamDateFormatter {
	...
	add({year = 0, week = 0, day = 0, hour = 0, minute = 0, second = 0}) {
		...
		if (year) {
			...
			let targetYear = currentYear + year;
	...
	static calculateTimestampFromString(string) {
		...
		const hour = parseInt(match[7] || 0, 10);
		const minute = parseInt(match[9] || 0, 10);
		const second = parseInt(match[11] || 0, 10);
