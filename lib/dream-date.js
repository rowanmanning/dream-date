...
class DreamDate extends DreamDateFormatter {
	...
	constructor(year, month, date, hour, minute, second) {
		...
		if (typeof year === 'number' && month === undefined) {
			this.timestamp = year;
		} else if (typeof year === 'string' && month === undefined) {
			this.timestamp = this.constructor.calculateTimestampFromString(year);
		} else {
			this.timestamp = this.constructor.calculateTimestampFromDateComponents(year, month, date, hour, minute, second);
		}
	}
	...
	static calculateTimestampFromDateComponents(year = 1, month = 1, date = 1, hour = 0, minute = 0, second = 0) {
		this.assertValidDateComponents(...arguments);
	...
	static assertValidDateComponents(year, month, date, hour, minute, second) {
		if (year <= 0) {
