 ...
 * @extends DreamDateFormatter
 *
 * @example <caption>Extend and create a date object</caption>
 * class MyDate extends DreamDate {};
 * const date = new MyDate('2018-04-23');
 */
class DreamDate extends DreamDateFormatter {
	...
	get monthName() {
		const month = this.constructor.schema.calendar.year.months[this.monthIndex];
		return (month ? month.name : '');
	}

	get monthAbbreviation() {
		const month = this.constructor.schema.calendar.year.months[this.monthIndex];
		return (month ? month.abbr : '');
	}

	get monthLongName() {
		const month = this.constructor.schema.calendar.year.months[this.monthIndex];
		return (month ? month.long : '');
	}
	...
	get dayName() {
		const day = this.constructor.schema.calendar.week.days[this.dayIndex];
		return (day ? day.name : '');
	}
